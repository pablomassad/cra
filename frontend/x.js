...
if (envArr.indexOf(env) === -1) {
    ...
    console.log('Debe elegir algun destino: desa / prod ')
    exit()
}
...
function pointTo () {
    ...
    /// /////////////////////////////////////////////////////////////////////////////
    console.log('==============================')
    console.log(`Lee environments.${env}.js`)
    const bufferEnv = fs.readFileSync(path.join(__dirname, `/src/environments.${env}.js`))
    let strEnv = bufferEnv.toString()

    const reg = /(versionName: ')(([0-9]{1}.[0-9]{1,2}.)([0-9]{2}))/gm
    strEnv = strEnv.replace(reg, ($0, $1, $2, $3, $4) => {
        // console.log('$0', $0) // versionName: '1.01.02
        // console.log('$1', $1) // versionName: '
        // console.log('$2', $2) // 1.01.02
        // console.log('$3', $3) // 1.01.
        // console.log('$4', $4) // 02
    ...
    fs.writeFileSync(path.join(__dirname, `/src/environments.${env}.js`), strEnv, err => {
        if (err === null) {
            console.log('Actualizacion deploy/environments.js OK')
        } else {
            console.log('Error actualizando environments.js: ', err)
        }
    })

    console.log('Copiando => src/environments.js')
    fs.copyFileSync(
        path.join(__dirname, `/src/environments.${env}.js`),
...
function initApk () {
    ...
    pkgJson.productName = apkName + '.' + env.toUpperCase()
...
function initAndroid () {
    ...
    let appId = ''
    const regAppId = /(?<=(setec))(.\w*)/gm
    strGradle = strGradle.replace(regAppId, ($0, $1, $2, $3) => {
        appId = (env === 'prod') ? '' : '.' + env
        if ($0 === '"') appId = appId + '"'
        return appId
    })
    console.log('new applicationId => ' + appId)
    ...
    const appNameEnv = env === 'prod' ? '' : ' ' + env.toUpperCase() // .charAt( 0 ).toUpperCase() + env.slice( 1 )

    const regAppName = 'app_name">(.*)<'
    const lblApp = '"app_name"'
    strStrings = strStrings.replace(lblApp + '>' + strStrings.match(regAppName)[1] + '<', lblApp + '>' + appNm + appNameEnv + '<')

    const regAppActName = 'title_activity_main">(.*)<'
    const lblAct = '"title_activity_main"'
    strStrings = strStrings.replace(lblAct + '>' + strStrings.match(regAppActName)[1] + '<', lblAct + '>' + appNm + appNameEnv + '<')
...
function signApk () {
    ...
    if (mode === 'r') {
        try {
            const apkPath = `./android/app/build/outputs/apk/release/${apkName}.${env}.apk`
        ...
        execSync(`${buildToolsPath}zipalign -v 4 ./android/app/build/outputs/apk/release/app-release-unsigned.apk ./android/app/build/outputs/apk/release/${apkName}.${env}.apk`, {
            stdio: 'inherit'
        })

        console.log('Ejectua apksigner')
        execSync(`${buildToolsPath}apksigner sign --ks keystore.jks --ks-pass pass:"Telecom.01" ./android/app/build/outputs/apk/release/${apkName}.${env}.apk`, {
            stdio: 'inherit'
        })
    } else {
...
function deployApk () {
    ...
    /// /////////////////////////////////////////////////////////////////////////////
    // const dest = `/Users/pablin/My Drive/Work/PyP Soft/SETEC/${apkName.toLocaleLowerCase()}.${env}.${newVersionName$}.apk`
    // const dest = `./public/${apkName.toLocaleLowerCase()}.${env}.apk`
    const dest = `/Users/pablin/My Drive/Work/PyP Soft/CRA/${apkName.toLocaleLowerCase()}.${env}.apk`
    console.log('Copiando apk a: ', dest)
    if (mode === 'r') {
        fs.copyFileSync(
            `./android/app/build/outputs/apk/release/${apkName.toLocaleLowerCase()}.${env}.apk`,
            dest
        )
    }
    if (mode === 'd') {
...
function deployWeb () {
    ...
    console.log('')
    console.log('###########################')
    console.log('DEPLOY APK TO WEBSITE ' + env)
    console.log('###########################')
